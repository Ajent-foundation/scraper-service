...
async function selectFileFromDialogHandler(
	...
	try {
		...
		await selectFileFromDialog(
			res.log,
			{ ...(res.locals.importantHeaders ? res.locals.importantHeaders : {}) },
			baseUrl,
			req.body.fileName,
		);
		return UTILITY.EXPRESS.respond(res, 200, { success: true });
	} catch (error) {
...
export default selectFileFromDialogHandler;
