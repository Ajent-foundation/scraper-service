...
import { keyboardControl, getKeyboardState } from '../../apis/node';
import { z } from 'zod';
...
export const RequestBodySchema = z.object({
	...
	modifiers: z.array(z.enum(['ctrl', 'alt', 'shift', 'super'])).optional(),
	type: z.enum(['keydown', 'keyup', 'type']).optional(),
	delayMin: z.number().optional(),
	delayMax: z.number().optional(),
});

export const RequestQuerySchema = z.object({
	action: z.enum(['state']).optional(),
});
...
async function keyboard(
	...
	try {
		...
		if (req.query.action === 'state') {
			const data = await getKeyboardState(
				res.log,
			...
			const data = await keyboardControl(
				res.log,
				{ ...(res.locals.importantHeaders ? res.locals.importantHeaders : {}) },
				baseUrl,
				req.body,
