...
import { getWindows, controlWindow } from '../../apis/node';
import { z } from 'zod';
...
export const RequestBodySchema = z.object({
	...
	action: z.enum(['focus', 'minimize', 'maximize', 'restore', 'close']).optional(),
});

export const RequestQuerySchema = z.object({
	action: z.enum(['list', 'control']).optional(),
});
...
async function windows(
	...
	try {
		...
		if (req.query.action === 'control') {
			const data = await controlWindow(
				res.log,
