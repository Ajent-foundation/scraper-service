...
import { downloadFile } from '../../apis/node';
import { z } from 'zod';
...
async function downloadFileFromSession(
	...
	try {
		...
		if (session) {
			...
			return UTILITY.EXPRESS.respond(
				res,
				200,
				await downloadFile(res.log, baseUrl, req.body.fileNames),
			);
		} else {
		...
		}, "session:downloadFileFromSession:39");
...
export default downloadFileFromSession;
